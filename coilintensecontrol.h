...
#ifndef COILINTENSECONTROL_H
#define COILINTENSECONTROL_H
...
void currentposition(int currentposx, int currentposy){
    ...
    for (i = 1; i < n; ++i)
    {
        ...
        //printf("Close coil force 1: %lf", closecoilforce);
            ...
            vecmag = cos(fabs(theta)) * fmax(farcoilforce, 100.0) + cos(fabs(phi)) * (closecoilforce);
        //printf("Vec mag: %lf\n", vecmag);
