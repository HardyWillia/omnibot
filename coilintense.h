...
void currentposition(int currentposx, int currentposy, double x, double y){
    ...
    for (i = 1; i < n; ++i)
    {
        ...
        farcoilforce = sin(x) * 30;
        closecoilforce = (sin(abs(x) * 30)) / (sin(abs(y)));
        ...
        vecmag = cos(abs(x)) * fmax(farcoilforce, 100.0) + cos(abs(y)) * (closecoilforce);
